...
class LatestCoinRecordManager {
  ...
  async setLatestCoinRecords(): Promise<void> {
    ...
    for (const coinId of Object.values(CoinId)) {
      const latestCoin = await Coin.findOne({ coinId }).sort({ createdAt: -1 }).exec();

      if (latestCoin) {
        const createdAt = (latestCoin as ICoinModel).createdAt;
        await this.redis.set(`latest_coin_record:${coinId}`, createdAt.toISOString());
        logger.debug(`Set latest coin record for ${coinId}: ${createdAt.toISOString()}`);
      } else {
  ...
  async getLatestCoinRecordTime(coinId: CoinId): Promise<string> {
    ...
    if (latestRecord) {
      ...
      logger.warn(`No latest record found for ${coinId}, using current time: ${currentTime}`);
      return currentTime;
    }
  }
